...
{ 
  ...
  cout << "1) Perform the equilibration of the NVT system before starting the real simulation or print the istant values " << endl;
...
//Equilibration and analysys with istant values (autocorrelation...)
  if (answer==1){
	for(int istep=1; istep <= nequistep; ++istep)
	{
		...
		Print_Istant(istep); //  istant value of potential energy and pressure
...
//update of the histogram of g(r)
     for (bin=igofr; bin<n_props; bin++){
		if (dr>(bin-2)*bin_size  && dr<(bin-1)*bin_size){
...
void Print_Istant(int istep)
{
   ofstream Epot, Pres;

   Epot.open("output.epot_ac_solid",ios::app);
   //Epot << "   " << istep << "   " << walker[iv]/(double)npart + vtail << endl;
   Epot << walker[iv]/(double)npart + vtail << endl;
   Epot.close();

   Pres.open("output.pres_ac_solid",ios::app);
   //Pres << "   " istep << "   "  << rho * temp + (walker[iw] + (double)npart * ptail) / vol << endl;
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    Epot.open("output.epot_liquid",ios::app);
    Pres.open("output.pres_liquid",ios::app);
    Gofr.open("output.gofr_liquid_NVT",ios::app);
    Gave.open("output.gave_liquid_NVT",ios::app);
	...
	r = (i-2)*bin_size;
	gdir = blk_av[i]/blk_norm/(rho*npart*(4./3.)*pi*(pow(r+bin_size,3)-pow(r,3))); //Radial istribution function
	glob_av[i] += gdir;
