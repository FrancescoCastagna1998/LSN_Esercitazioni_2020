...
{
	double cost = -5./2.;
	return pow(val,4) + cost*pow(val,2);
}

double external_potential_prime(double val)
{
	double cost = -5./2.;
	return 4*pow(val,3) + 2*cost*val;
}

double external_potential_second(double val)
{
	double cost = -5./2.;
	return 12*pow(val,2) + 2*cost;
}
...
double variationalWaveFunction(double v)
{
	...
	double mu = 0.81;
	double sigma = 0.62;
	double esp_sum = (-1.)*pow((v + mu),2)/(2*pow(sigma,2));
	double esp_diff = (-1.)*pow((v - mu),2)/(2*pow(sigma,2)); 
...
double variationalWaveFunction_second(double v)
{
	...
	double mu = 0.81;
	double sigma = 0.62;
	double esp_sum = (-1.)*pow((v + mu),2)/(pow(sigma,2));
	double esp_diff = (-1.)*pow((v - mu),2)/(pow(sigma,2));
	double a = 1/(pow(sigma,2));
...
{
	ofstream out("potential_PIMC_constant_T1.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("kinetic_PIMC_constant_T1.dat");
	for(int i=0;i<timeslices;i++)
	{
...
{
	ofstream out("probability_PIMC_constant_T1.dat");
