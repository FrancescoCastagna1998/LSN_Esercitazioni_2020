...
void Individual::Gen_Mut_2(int index1 ,Random* rnd, int contigcity){  //contigcity must be in [0,(m_Ind.size())/2-1)
	City City1;
	City1 = m_Ind[0];
	m_Ind.erase(m_Ind.begin()); // remove the first in order to preserve the position of the first city
	int ncity = (int) m_Ind.size(); // ncity - the first
	int index2 = (int)((m_Ind.size()-2.*contigcity)*(rnd->Rannyu())); // to avoid ovrelaps between the m contiguos city
	for (int icontig = 0; icontig< contigcity; icontig++){
		iter_swap (m_Ind.begin()+ Pbc(index1 + icontig,ncity),m_Ind.begin()+ Pbc(index2 + icontig,ncity));
	}
	m_Ind.insert(m_Ind.begin(), City1);  //after mutation reinsert the first city in the position 1 of the path	
}
void Individual::Gen_Mut_3(int index ,int contigcity){ //contigcity must be in  [0,m_ind.size()-1)
	City City1;
	City1 = m_Ind[0];
	m_Ind.erase(m_Ind.begin());// remove the first in order to preserve the position of the first city
	int ncity = (int) m_Ind.size(); // ncity - the first
	if ((long unsigned int)(index + contigcity) >= m_Ind.size()){
		rotate(m_Ind.begin(),m_Ind.begin() + Pbc(index + contigcity + 1,ncity),m_Ind.end());
		reverse( m_Ind.begin() + index - Pbc(index + contigcity + 1,ncity), m_Ind.end());
		rotate(m_Ind.begin(),m_Ind.end() - Pbc(index + contigcity + 1,ncity),m_Ind.end());
	}
	else{
		reverse( m_Ind.begin() + index, m_Ind.begin() + index + contigcity + 1);
	}
	m_Ind.insert(m_Ind.begin(), City1); //after mutation reinsert the first city in the position 1 of the path	
}
